...
transaction. A high level overview of transactional states:
  ...
  * Ready: the transactional ID is ready for a new transaction
  * Ongoing: the transactional ID is being used in a began transaction
  * Preparing / Prepared: a commit is in progress
  * Aborting: an abort is in progress
  * Killing: the transactional ID is timing out
  * Tombstone: the transactional ID has expired and is not in use
